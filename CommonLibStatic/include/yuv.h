...
private:
	...
	int        height;
	int        width;
	// height of U and V depending on chroma format
	int        heightUV;
	// width of U and V depending on chroma format
	int        widthUV;
	// pic size for Y
	int        picsizeY;
	// pic size for each U and V
	int        picsizeUV;
	// YUV size
	int        size_in_byte;
	// chroma format: 444, 422, 420 or 400
	int        sampling;
...
CIYuv<PixelType>::CIYuv(int h, int w, int chroma_format)
{
	...
	//only for m_pcImageLeft and m_pcImageRight
	if (cfg.getSynthesisMode() == 0) // General mode
	{
		setUpsampleFilter(cfg.getFilter(), cfg.getPrecision());
	}
	else // 1D mode: No upsampling is done before going into Visbd
	{
		setUpsampleFilter(cfg.getFilter(), 1);
	}
}
...
/**
	malloc comp (Y, U, V) and the 1D pBuffer
*/
template <class PixelType>
