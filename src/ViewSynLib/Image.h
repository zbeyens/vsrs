...
public:
	...
	*/
	bool convertYUVToYUV444(shared_ptr<Image<PixelType>> src);

	/*!
		Convert the mat in YUVXXX to BGR format. Used at the beginning of the pipeline.
		@param src	image in YUVXXX format to convert into this image
	*/
	bool convertYUVToBGR(shared_ptr<Image<PixelType>> src);
	...
	*/
	void convertMatToBuffer1D(shared_ptr<Image<PixelType>> src, int channels);
...
private:
	...
	*/
	...
	void convertYUV400ToYUV444(shared_ptr<Image<PixelType>> src);
	void convertYUV420ToYUV444(shared_ptr<Image<PixelType>> src);
	void convertYUV422ToYUV444(shared_ptr<Image<PixelType>> src);
	void convertYUV444ToYUV444(shared_ptr<Image<PixelType>> src);

	void convertYUV400ToBGR(shared_ptr<Image<PixelType>> src);
	void convertYUV420ToBGR(shared_ptr<Image<PixelType>> src);
	void convertYUV422ToBGR(shared_ptr<Image<PixelType>> src);
	void convertYUV444ToBGR(shared_ptr<Image<PixelType>> src);
...
template <class PixelType>
bool Image<PixelType>::initYUVFromMat()
{
	...
	for (int h = 0; h < sh; h++)
	{
		if ((Y[h] = (PixelType *)malloc(sw * sizeof(PixelType))) == NULL
			|| (U[h] = (PixelType *)malloc(sw * sizeof(PixelType))) == NULL
			|| (V[h] = (PixelType *)malloc(sw * sizeof(PixelType))) == NULL) return false;
	}

	int nChannels = m_imageMat->nChannels;
	...
	for (int h = 0; h < sh; h++)
	{
		for (uint w = 0; w < sw; w++)
		{
			if (nChannels == 3)
			{
				Y[h][w] = (PixelType)(m_imageMat->imageData[(h*sw + w) * nChannels]);
				U[h][w] = (PixelType)(m_imageMat->imageData[(h*sw + w) * nChannels + 1]);
				V[h][w] = (PixelType)(m_imageMat->imageData[(h*sw + w) * nChannels + 2]);
			}
			else
			{
				Y[h][w] = (PixelType)(m_imageMat->imageData[(h*sw + w)]);
				U[h][w] = (PixelType)(m_imageMat->imageData[(h*sw + w)]);
				V[h][w] = (PixelType)(m_imageMat->imageData[(h*sw + w)]);
			}
		}
	}
...
template <class PixelType>
bool Image<PixelType>::convertYUVToBGR(shared_ptr<Image> src)
{
...
template <class PixelType>
void Image<PixelType>::convertYUV400ToBGR(shared_ptr<Image> src)
{
...
template <class PixelType>
void Image<PixelType>::convertYUV420ToBGR(shared_ptr<Image> src)
{
	int h, w, cH, cW;
	int ir, ig, ib;
...
template <class PixelType>
void Image<PixelType>::convertYUV422ToBGR(shared_ptr<Image> src)
{
	int h, w, cH, cW;
	int ir, ig, ib;
...
template <class PixelType>
void Image<PixelType>::convertYUV444ToBGR(shared_ptr<Image> src)
{
	int h, w, cH, cW;
	int ir, ig, ib;
	for (h = 0; h < m_height; h++)
...
template <class PixelType>
bool Image<PixelType>::convertYUVToYUV444(shared_ptr<Image> src)
{
	if (m_sampling != 444 || Y == NULL || m_height != src->getHeight() || m_width != src->getWidth()) return false;
...
template <class PixelType>
void Image<PixelType>::convertYUV400ToYUV444(shared_ptr<Image> src)
{
	memset(U[0], 128, m_picsizeUV * 2);
}

template <class PixelType>
void Image<PixelType>::convertYUV420ToYUV444(shared_ptr<Image> src)
{
...
template <class PixelType>
void Image<PixelType>::convertYUV422ToYUV444(shared_ptr<Image> src)
{
	int h, dstH, dstW, srcH, srcW;
...
template <class PixelType>
void Image<PixelType>::convertYUV444ToYUV444(shared_ptr<Image> src)
{
...
template <class PixelType>
void Image<PixelType>::convertMatToBuffer1D(shared_ptr<Image<PixelType>> src, int channels)
{
